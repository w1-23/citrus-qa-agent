...
_CARDS: List[dict] = [
    ...
    {"id": "retrieval_008", "type": "retrieval", "name": "品种属性检索",
     "description": "品种/性状/登记号问题优先命中UCR品种库",
     "prompt": "涉及柑橘品种、登记号(CRC/PI)、亲本、来源、性状描述时，优先使用citrus_rag_search并关注source=ucr的品种条目(registry_id/variety_name)。"},
    ...
    {"id": "retrieval_010", "type": "retrieval", "name": "RAG优先降级",
     "description": "本地覆盖不足→如实声明缺口",
     "prompt": "先用citrus_rag_search检索本地文献库。如果返回很少或无具体数据，如实声明本地库覆盖缺口，不编造外源结论；已开启联网搜索时可补充上限1次联网检索。"},
