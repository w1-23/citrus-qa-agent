...
**问题（zh）**：该研究的所有表达构建体使用哪种克隆系统构建？HA6 与 MYB308 的 CDS 被克隆到哪些表达载体中？

**问题（en）**：Which cloning system was used to assemble all the expression constructs in this study, and into which expression vectors were the CDSs of HA6 and MYB308 cloned?

**gold_note（人工待核对）**：c36：'all the constructs were made using the Golden Gate cloning system'；HA6 与 MYB308 的 CDS 克隆到 BF-SXGA 或 H2gE-35SA 表达载体。
...
## cor-051 | L2 | 功能 | paper5/paper_b654164f_000466_F_2016_BMC_Genomics_Ide@31

**问题（zh）**：潜在 miRNA 靶基因（311 个转录本）的功能涉及哪些类别，哪些保守 miRNA 家族被识别？

**问题（en）**：Which functional categories cover the 311 potential miRNA target transcripts, and which conserved miRNA families were identified?

**gold_note（人工待核对）**：生长发育/信号转导/相变/代谢/蛋白转运/胁迫响应；miR156/159/160/164/167/172/396/482
...
## cor-052 | L3 | 统计 | paper6/Evolutionary_2026_origin_neohesperidoside_bitter_metabolite_and@58

**问题（zh）**：为研究新橙皮苷苦味代谢物的起源，研究者采集了多少份柑橘种质、覆盖哪些属，采果期约在花后多少天？

**问题（en）**：To study the origin of neohesperidoside bitter metabolites, how many citrus accessions covering which genera were collected, and at approximately how many days post anthesis were fruits harvested?

**gold_note（人工待核对）**：38 份，覆盖 C. lansium、A. buxifolia 等早分支属；约 230 DPA
...
## cor-053 | L1 | 实体 | categories-cn/doc_hong_kong_fortunella_hindsii_crc_3790_crc_3790@8

**问题（zh）**：Riverside 保存的两份金柑 F. hindsii 登录号（PI 539723、PI 539724）最终染色体计数结论是什么？

**问题（en）**：What was the final chromosome count conclusion for the two F. hindsii accessions (PI 539723 and PI 539724) maintained at Riverside?

**gold_note（人工待核对）**：两份均为二倍体 2n=18（Prof. Soost 信函确认）
...
## cor-065 | L3 | 功能 | paper4/paper_24410712_000324_F_2022_Antioxidants__Ba@28

**问题（zh）**：添加柑橘副产物纤维（0.5%-2%）对博洛尼亚香肠的残留亚硝酸盐、色泽和质构有何影响？

**问题（en）**：How did the addition of citrus by-product fiber (0.5%-2%) affect residual nitrite, color and texture of bologna sausage?

**gold_note（人工待核对）**：显著降低残留亚硝酸盐，改变色泽与质构参数（亮度增加）
...
## cor-066 | L3 | 统计 | paper6/Elucidation_2026_flavor_codes_citrus_fruit_peels@62

**问题（zh）**：在 DSPC 风味网络中，哪个代谢物因度数 11 与介数中心性 214.27 被识别为最具影响力的代谢物？

**问题（en）**：Which metabolite was identified as the most influential in the DSPC flavor network, with a degree of 11 and a betweenness centrality of 214.27?

**gold_note（人工待核对）**：SCS（典型柑橘风味特征代谢物）

**语料原文片段（证据，前 900 字）**：

> # Correlation coefficients with t  
> Fig. 6. (continued).  
