...
| 💬 **流式回答 + 思维链** | 回答逐 token 上屏；DeepSeek 深度思考（reasoning）实时展示在回答上方的「深度思考」折叠条 |
...
- **左侧**：会话区（列表/切换/新建 ＋/重命名/删除）+ 工作区（写作成果）+ 文献引用，手风琴并列收缩
- **中间**：对话区（用户气泡右侧 / 回答气泡左侧，流式回答、深度思考折叠条、工具执行记录可展开）
- **右上角**：上下文占用环（点击看上下文细分明细）、主题切换
- **输入框**：左下角权限图标（🔓 全权 / 🛡️ 询问 / 🔒 只读，点击切换），右下角发送 ⇄ 停止
