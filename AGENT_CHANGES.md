...
- **v8.4.14（2026-08-14）**：测试环境性失败根治 + 死代码清理 + 路径安全修复——①**测试沙箱规避**：新增 tests/_tmpenv.py（工作区内 tests/.tmp_runner/ 统一临时路径），conftest 的 CITRUS_LOG_DIR 与 10 个测试文件的 `tempfile.mkdtemp()` 全部迁移——此前 DSH 沙箱下系统 TEMP 不可写导致 16 个"环境性失败"（sqlite unable to open database file 级联断言失败），迁移后 **16→1**（仅剩 fastembed 模型缓存项，用户 rag-agent 环境有缓存全绿）②**路径安全修复（真实漏洞）**：analyze.py/readfile.py 的 `os.path.commonpath` 同盘前缀漏洞（E:\anywhere 与 E:\agent 同盘前缀会被放行）→ `is_relative_to` 严格判定；search.py pdf_read 的 `startswith(workspace_root)` 无路径边界（workspace_evil 可绕过）→ is_relative_to；test_ag10_limits 改用工作区内项目根外路径 ③**死代码清理**：删除零引用函数 21 个（search._filter_web_results、cache_metrics.cache_stats、llm_pool.clear_llm_pool、progress_bus.enable_sse_debug/get_tool_name、skill_tree.get_search_prompt/get_stats、loader.available_agents/available_formats、config.get_available_models/get_deepseek_client、gpu_lock.acquire_gpu、multi_retriever.count_points/_reset_singleton_for_testing、memory 的 extract_entities/extract_entity_relations/get_all_preferences/get_concepts/get_preference/get_recent_entities）；FeatureFlags 类（0 个开关）+ /api/v1/flags、/api/v1/flags/toggle、/api/v1/memory 死端点；context_manager build_human_message 的 history_summary 死分支（字段从未赋值，摘要已嵌入视图消息）；前端 executeBuild 函数 + .build-exec-btn 死 CSS + token 死事件 case（tool_progress 经核验为活事件保留）；整个 src/cli/（零引用）、pipeline_utils.py（唯一函数死）、tests/simulate_flow.py、tests/verify_context_budget.py；版本 8.4.13→8.4.14；全量 102 passed / 1 failed（仅 fastembed 缓存）
